// ============================================================================
//  Squash (Gaelco) — squash_video_top.v: TOP de vídeo (timing + tilemaps + paleta).
//
//  Une squash_video_timing + squash_video (2 tilemaps con prioridad Tipo-1) + paleta
//  (xBGR_555) y entrega RGB 5-5-5 + sync/blank/DE + vblank_irq.
//
//  FASE 4d (parcial): camino de TILEMAP completo. Los SPRITES (squash_sprite_engine, fase 4c)
...
module squash_video_top #(
    ...
    // tilemap L0/L1: videoram (de squash_vmem) + gfx (SDRAM)
    ...
    // paleta tilemap (puerto A) + sprite (puerto B) (de squash_vmem)
    output wire [9:0]  pal_a,   input wire [15:0] pal_q,
    output wire [9:0]  palb_a,  input wire [15:0] palb_q,
    // spriteRAM + gfx de sprites (de squash_vmem / SDRAM)
    ...
    // CALIBRADO SQUASH (2026-06-21) vs MAME (pen=c12<<3|c11<<2|c10<<1|c09, planos {0,1/4,2/4,3/4}).
    ...
    wire [4:0] r5, g5, b5;
    squash_palette u_pal (.pal_word(pal_q), .r(r5), .g(g5), .b(b5));
    ...
    squash_sprite_layer #(.VTOTAL(VTOTAL)) u_spr (
