// ============================================================================
//  Thunder Hoop (Gaelco) — thoop_video_top.v: TOP de vídeo (timing + tilemaps + paleta).
//
//  Une thoop_video_timing + thoop_video (2 tilemaps con prioridad Tipo-1) + paleta
//  (xBGR_555) y entrega RGB 5-5-5 + sync/blank/DE + vblank_irq.
//
//  FASE 4d (parcial): camino de TILEMAP completo. Los SPRITES (thoop_sprite_engine, fase 4c)
...
module thoop_video_top #(
    ...
    // tilemap L0/L1: videoram (de thoop_vmem) + gfx (SDRAM)
    ...
    // paleta tilemap (puerto A) + sprite (puerto B) (de thoop_vmem)
    output wire [9:0]  pal_a,   input wire [15:0] pal_q,
    output wire [9:0]  palb_a,  input wire [15:0] palb_q,
    // spriteRAM + gfx de sprites (de thoop_vmem / SDRAM)
