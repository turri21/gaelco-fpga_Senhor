// ============================================================================
//  Alligator Hunt (Gaelco, Tipo-2 gaelco2.cpp / chip GAE1) — wrally2_vmem.v
//
...
module wrally2_vmem (
    ...
    input  wire [14:0] spr_a,            // word para el motor de SPRITES izdo (lista + indirección)
    output wire [15:0] spr_q,
    input  wire [14:0] spr2_a,           // word para el 2º motor de SPRITES (TWIN, monitor derecho)
    ...
    // motor de sprites (izdo)
    (* ramstyle = "no_rw_check" *) reg [7:0] se_hi[0:16383], se_lo[0:16383], so_hi[0:16383], so_lo[0:16383];
    // motor de sprites #2 (TWIN, monitor derecho) — copia idéntica leída por el 2º motor @clk96
    ...
    wire        s_par  = spr_a[0];     // puerto del motor de SPRITES izdo (lista + indirección)
    wire [13:0] s_hidx = spr_a[14:1];
    wire        s2_par  = spr2_a[0];   // puerto del 2º motor de SPRITES (twin, derecho)
    ...
    always @(posedge clk96) begin     // LECTURA del motor de sprites izdo @96
    ...
    // 2º motor de SPRITES (twin, derecho): copia idéntica, ESCRITURA CPU @48 + LECTURA @96
    reg [7:0] s2e_hi_q, s2e_lo_q, s2o_hi_q, s2o_lo_q; reg s2_par_q;
    always @(posedge clk) begin       // ESCRITURA CPU @48 (misma que la copia izda)
    ...
    always @(posedge clk96) begin     // LECTURA del 2º motor @96
